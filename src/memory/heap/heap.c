...
static int heap_validate_table(void* ptr, void* end, struct heap_table* table)
{
    ...
    size_t total_blocks = table_size / SHEAROS_HEAP_BLOCK_SIZE;
...
{
    return ((unsigned int)ptr % SHEAROS_HEAP_BLOCK_SIZE) == 0;
}
...
{
    if(val % SHEAROS_HEAP_BLOCK_SIZE == 0)
    {
        return val;
    }

    val = (val - (val % SHEAROS_HEAP_BLOCK_SIZE));
    val += SHEAROS_HEAP_BLOCK_SIZE;
    return val;
}
...
{
    return heap->saddr + (block* SHEAROS_HEAP_BLOCK_SIZE);
}
...
void* heap_malloc(struct heap* heap, size_t size)
{
    ...
    uint32_t total_blocks = aligned_size / SHEAROS_HEAP_BLOCK_SIZE;
