...
static struct process* processes[SHEAROS_MAX_PROCESSES] = {};
...
{
    if (process_id < 0 || process_id >= SHEAROS_MAX_PROCESSES)
    {
        return NULL;
    }
...
{
    int res = 0;
    paging_map_to(process->task->page_directory, (void*) SHEAROS_PROGRAM_VIRTUAL_ADDRESS, process->ptr, paging_align_address(process->ptr + process->size), PAGING_IS_PRESENT | PAGING_ACCESS_FROM_ALL | PAGING_IS_WRITEABLE);
    return res;
}
...
{
    for (int i = 0; i < SHEAROS_MAX_PROCESSES; i++)
    {
        if (processes[i] == 0)
            return i;
    }
...
int process_load_for_slot(const char* filename, struct process** process, int process_slot)
{
    ...
    program_stack_ptr = kzalloc(SHEAROS_USER_PROGRAM_STACK_SIZE);
...
static struct process_allocation* process_get_allocation(struct process* process, void* addr)
{
    for(int i = 0; i < SHEAROS_MAX_PROGRAM_ALLOCATIONS; ++i)
    {
...
void process_free(struct process* process, void* ptr)
{
    ...
    struct process_allocation* allocation = process_get_allocation(process, ptr);
    if(!allocation)
    {
        return;
    }
