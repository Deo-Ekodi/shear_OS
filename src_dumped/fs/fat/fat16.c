...
/**
 * function prototypes for filesystem
 */
...
static int fat16_get_fat_entry(struct disk* disk, int cluster)
{
    ...
    res = diskstreamer_seek(stream, fat_table_position * (cluster * SHEAROS_FAT16_FAT_ENTRY_SIZE));
    if (res < 0)
    {
        goto out;
    }
...
static int fat16_get_cluster_for_offset(struct disk* disk, int starting_cluster, int offset)
{
    ...
    for (int i = 0; i < clusters_ahead; i++)
    {
        ...
        if (entry == SHEAROS_FAT16_BAD_SECTOR)
        {
            res = -EIO;
            goto out;
        }
...
struct fat_item* fat16_find_item_in_directory(struct disk* disk, struct fat_directory* directory, const char* name)
{
    ...
    char tmp_filename[SHEAROS_MAX_PATH];
...
static void fat6_free_file_descriptor(struct fat_file_descriptor* desc)
{
...
int fat16_close(void* private)
{
    fat6_free_file_descriptor((struct fat_file_descriptor*) private);
    return 0;
}
